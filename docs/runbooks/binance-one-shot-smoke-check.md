...
- Producer graceful SIGINT/SIGTERM, bounded final Kafka flush, and long-running WebSocket reconnect are implemented and tested separately. This one-shot helper calls `receive_one_binance_trade_event()`, which only opens a WebSocket, receives one event, parses it, and closes; it does not create a `KafkaPublisher` or publish to Kafka. The default publisher callback-result path is implemented and covered by the separate Kafka smoke runbook, while callback failure/missing-result behavior is not exercised here. Monitoring, throughput optimization, idempotence, replay, backfill, and exactly-once guarantees remain unimplemented.
